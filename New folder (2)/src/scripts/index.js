 ...
 if(key === 13)  // the enter key code
  {
    ...
    var serverValidationForgotPassword = function () { //setting the new password on the database
        // db.accessData("getDataByAttribute","admin",{attribute:{id:$("#forgot-password-username").val()}},function(){});
        return true;
    }
    ...
    function sendNewPassword() { //called to send the new passwored to the user
        if (serverValidationForgotPassword()) {
            $("#new-password-sent").removeClass("display-none-class");
            $("#forgot-password-form").addClass("display-none-class");
            var newPassword = newPasswordGenerate();
            event.preventDefault();
            // emailjs.send("mentormenteeXT2017@gmail.com", "password", { "email":"bvrs95@gmail.com", "to_name": "Subbu", "new_password": "<b>" + newPassword + "</b>" });
        }
    }
